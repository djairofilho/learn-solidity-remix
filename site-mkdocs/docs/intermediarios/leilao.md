...
Sistema de leilão: maior lance vence.
 ...
 * 5. Após o tempo, chame encerrarLeilao() e confirme evento de encerramento.
 */
contract Leilao {
    ...
    function fazerLance() public payable {
        ...
        // Se tinha um lance anterior do mesmo usuário, adiciona ao novo
...
1. **Deploy** com `_precoMinimo` = **100000000000000000** (0.1 ETH) e `_duracao` = **600** (10 min)
2. Troque para **Conta 2**, defina **Value = 0.2 ETH** e chame `fazerLance()`
3. Troque para **Conta 3**, defina **Value = 0.3 ETH** e chame `fazerLance()`
4. Chame `verMaiorLance()` → deve mostrar Conta 3
5. **Conta 2** chama `sacarLanceAnterior()` → recebe reembolso
6. Após tempo, **Conta 1** chama `encerrarLeilao()` → recebe o valor

## Use Case

NFT auctions • Presales • Leilões reversos
