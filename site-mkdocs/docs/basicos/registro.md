...
Sistema de registro de documentos (cartório blockchain).
 ...
 * 5. Chame obterTodosDocumentos() para inspecionar o histórico completo.
 */
contract Registro {
...
1. **Deploy** no Remix VM
2. Chame `registrar("doc-1")` → evento emitido
3. Chame `registrar("doc-2")`
4. Chame `totalDocumentos()` → deve mostrar **2**
5. Chame `obterDocumento(0)` → mostra primeiro documento
6. Chame `obterUltimoDocumento()` → mostra "doc-2"
7. Chame `obterTodosDocumentos()` → array com ambos

## Use Case

Aplicação real • Notarização • Certificação • timestamping blockchain
