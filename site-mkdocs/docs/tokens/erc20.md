# ERC20 - Token Fungível

Implementação didática do padrão ERC-20 para criar suas próprias criptomoedas.

**Conceitos ensinados:**
- `transfer()` e `transferFrom()` — enviar tokens
- `balanceOf()` — saldo de um endereço
- `approve()` e `allowance()` — autorização delegados
...
1. **Deploy** com `_initialSupply` = **1000**
2. Chame `balanceOf(sua_conta)` → **1000000000000000000000** (1000 * 10^18)
3. Chame `transfer(outro_endereco, 100000000000000000000)` → transfere 100 tokens
4. Outro endereço chama `balanceOf()` → confirma recebimento
5. Outro endereço chama `approve(sua_conta, 50000000000000000000)` (50 tokens)
6. Sua conta chama `transferFrom(outro_endereco, sua_conta, 30000000000000000000)` (30 tokens)

## Use Case

Criar seu próprio token (tipo seu próprio "Bitcoin") • Utility tokens • Governance tokens
